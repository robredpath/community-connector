...
class Command(BaseCommand):
	help = 'Runs the message queue, attempting to send any messages that have not yet been sent'
	...
	def handle(self, *args, **options):
		for message in Message.objects.filter(email_has_been_sent=False):
			for user in User.objects.all():
				...
				message.save()
				self.stdout.write(self.style.SUCCESS('Successfully sent email to "%s"' % user))
